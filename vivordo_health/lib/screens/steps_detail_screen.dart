...
class _StepsDetailScreenState extends State<StepsDetailScreen> {
  ...
  List<_StepDay> _rangeData(QuerySnapshot<Map<String, dynamic>>? snapshot) {
    final stepsByDay = <String, int>{};
    final distanceByDay = <String, double>{};
    for (final doc in snapshot?.docs ?? const []) {
      stepsByDay[doc.id] =
          ((doc.data()['steps'] as Map?)?['sum'] as num?)?.round() ?? 0;
      distanceByDay[doc.id] =
          ((doc.data()['distance'] as Map?)?['sum'] as num?)?.toDouble() ?? 0;
    }
  ...
  Widget _buildContent(
    ...
    final best = data.isEmpty
        ? null
        : data.reduce((a, b) => a.steps >= b.steps ? a : b);
      ...
      child: Column(
        children: [
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _iconBubble(Icons.directions_walk_rounded, _blue),
          ...
          Row(
            children: [
              Text('Goal ${NumberFormat.decimalPattern().format(goal)}'),
              const Spacer(),
  ...
  Widget _chartCard(List<_StepDay> data, double? usual) {
    ...
    return _card(
      ...
      child: SizedBox(
        height: 245,
        child: _StepsChart(
          values: data.map((day) => day.steps.toDouble()).toList(),
  ...
  Widget _distanceCard(double distanceKm) {
    final decimals = distanceKm == 0 || distanceKm >= 10 ? 1 : 2;
    return _card(
      padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 16),
      child: Row(
        children: [
          _iconBubble(Icons.route_rounded, const Color(0xFF20B26B)),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${distanceKm.toStringAsFixed(decimals)} km',
                  style: const TextStyle(
                    fontSize: 25,
                    fontWeight: FontWeight.w900,
                  ),
  ...
  Widget _breakdownCard(List<_StepDay> data, _StepDay? best) {
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Text(
                    NumberFormat.decimalPattern().format(ordered[i].steps),
                    style: const TextStyle(
                      color: _blue,
                      fontSize: 17,
                      fontWeight: FontWeight.w800,
...
class _StepDay {
  const _StepDay(this.date, this.steps, this.distanceKm);
  final DateTime date;
  final int steps;
...
class _StepsChartState extends State<_StepsChart> {
  ...
  @override
  void didUpdateWidget(covariant _StepsChart oldWidget) {
    ...
    canvas.drawPath(
      ...
      Paint()
        ..shader = ui.Gradient.linear(Offset.zero, Offset(0, height), [
          const Color(0xFF2878E8).withValues(alpha: .24),
          const Color(0xFF2878E8).withValues(alpha: 0),
        ]),
    );
    canvas.drawPath(
      line,

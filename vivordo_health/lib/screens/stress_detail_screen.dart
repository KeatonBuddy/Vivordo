...
class _StressDetailScreenState extends State<StressDetailScreen> {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _section(_rangeName),
          _chartCard(chart),
          _section('What\'s shaping your score'),
          _driversCard(today),
  ...
  Widget _driversCard(_StressDay latest) {
    if (latest.drivers.isEmpty) {
      return _card(
        padding: const EdgeInsets.all(20),
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            _iconBubble(Icons.auto_graph_rounded, _purple),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  const Text(
                    'Your signal breakdown is still learning',
                    style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800),
                  ),
                  const SizedBox(height: 5),
                  Text(
                    latest.justification ??
                        'As Vivordo gathers more readings, the signals shaping your score will appear here.',
                    style: TextStyle(
                      color: context.vivordoColors.textSecondary,
                      height: 1.35,
                    ),
    ...
    final drivers = latest.drivers.take(6).toList();
    return _card(
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Text(
              'Starting influence · personalized over time',
              style: TextStyle(
                color: context.vivordoColors.textSecondary,
                fontSize: 12,
              ),
          ...
          for (var index = 0; index < drivers.length; index++) ...[
            _driverRow(drivers[index], index),
  ...
  Widget _driverRow(_StressDriver driver, int index) {
    final color = [_red, _yellow, _purple, _green][index % 4];
    final percent = driver.influence == null
        ? null
        : driver.influence!.abs() <= 1
        ? driver.influence!.abs() * 100
        : driver.influence!.abs();
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 12),
      child: Row(
        children: [
          _iconBubble(_driverIcon(driver.name), color, size: 42),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
  ...
  IconData _driverIcon(String name) {
    final lower = name.toLowerCase();
    if (lower.contains('mood')) return Icons.mood_rounded;
    if (lower.contains('sleep') || lower.contains('recovery')) {
      return Icons.bedtime_rounded;
    }
    if (lower.contains('heart') || lower.contains('hrv')) {
      return Icons.monitor_heart_rounded;
    }
    if (lower.contains('breath') || lower.contains('oxygen')) {
      return Icons.air_rounded;
    }
    if (lower.contains('activity') || lower.contains('step')) {

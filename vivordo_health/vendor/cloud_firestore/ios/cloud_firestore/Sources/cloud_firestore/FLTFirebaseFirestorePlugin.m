...
- (void)transactionGetApp:(nonnull FirestorePigeonFirebaseApp *)app
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    @try {
      snapshot = [transaction getDocument:document error:&error];
    } @catch (NSException *exception) {
      // A delayed read may race transaction completion after suspension.
      if ([exception.reason containsString:@"A transaction object cannot be used after"]) {
        completion(nil, [FlutterError errorWithCode:@"aborted"
                                           message:@"Transaction expired before the read completed."
                                           details:nil]);
        return;
      }
      @throw;
